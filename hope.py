...
offs=60
K=10
data=pickle.load(open('databin/latih.p','rb'))
...
def CC(train,test,mn,mx):
    ...
    b=[max(numpy.correlate(train[i+1],test[i+1])) for i in range(3)]
...
def init(uji):
    ...
    for x in a[:K]:
        if x[2] in res:
                ...
                res[x[2]][1]+=x[0]
    try:
        return [(k,v[0]) for k,v in res.items()]#mode([x for x in a[:K]])
    except:
