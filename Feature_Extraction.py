...
def extraction(song, offset, duration):
    ...
    spec_centroid = librosa.feature.spectral_centroid(waveform, sr=sr)[
        0
    ]  # spectral centroid
    spec_rolloff = librosa.feature.spectral_rolloff(waveform + 0.01, sr=sr)[
        0
    ]  # spectral rolloff
    energy = numpy.array(
        [
            sum(waveform[i : i + frame_length] ** 2)
...
def run(dur, offs):
    ...
    result = [
        extraction("../Music/" + data + "128kbps/" + waveform, offs, dur)
        for waveform in file
    ]
    print(result[0])
    print(len(result))
