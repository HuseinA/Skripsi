...
clas = pickle.load(open("databin/songclass.p", "rb"))
...
def init(uji):
    ...
    distance = [distance[i] + [clas[x[1]]] for i, x in enumerate(distance) if x[1] in clas]
...
def main():
    ...
    print_result(test, index - 1)
    # testing()


def testing():  # Confusion Matrix Multiclass Problem
...
if __name__ == "__main__":
    ...
    if input("Show Song List? (y/n) : ") == "y":
        print_song()
    test = [test[i] + [clas[x[4]]] for i, x in enumerate(test) if x[4] in clas]
    main()
